...
description: Execute `flutter analyze --fatal-infos --fatal-warnings` to identify warnings and errors, and use `dart fix --apply` to automatically resolve mechanical lint issues. Use during development to ensure code quality and before committing changes.
metadata:
...
- [ ] 2. Run the analyzer using the CLI command `flutter analyze --fatal-infos --fatal-warnings`.
- [ ] 3. Review the diagnostic output.
- [ ] 4. Ensure all warnings and infos are resolved as failures.
